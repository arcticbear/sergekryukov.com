if ($) $ (function () {
  ...
  e2UpdateSubmittability = function () {
    ...
    $ ('#submit-button').attr ('disabled', shouldBeEnabled ? '' : 'disabled')
  }
  ...
  $e2AddImage = function (imageThumb, imageFull) {
    ...
    $newImage.css ('width', '')
    $newImage.find ('img.uploaded-image-preview')
      .attr ('src', imageThumb)
      .attr ('alt', imageFull)
    $newImage.find ('.uploaded-image-remover')
      .data ('imageThumb', imageThumb)
      .data ('file', imageFull)
    return $newImage
  }
  
  $ ('.uploaded-image-remover').css ('opacity', 0).show ()
  
  $ ('#e2-uploaded-image-prototype').mouseenter (function () {
    $ (this).find ('.uploaded-image-remover').stop ().fadeTo (333, 1)
  })

  $ ('#e2-uploaded-image-prototype').mouseleave (function () {
    $ (this).find ('.uploaded-image-remover').stop ().fadeTo (333, 0)
  })
  ...
  $ ('.uploaded-image-remover').click (function () {
    var $picToDelete = $ (this).parent ().parent ().parent ()
  ...
  $ ('#e2-uploaded-images').children ().each (function () {
    imageThumb = $ (this).find ('img.uploaded-image-preview').attr ('src')
    imageFull = $ (this).find ('img.uploaded-image-preview').attr ('alt')
    $ (this).remove ()
